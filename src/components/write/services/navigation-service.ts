...
const NAV_FILE_PATH = 'src/data/navigation.yaml'
  ...
  try {
    ...
    if (pendingAvatars && pendingAvatars.length > 0) {
      ...
      for (const { catIndex, itemIndex, file } of pendingAvatars) {
        ...
        const avatarPath = `public/images/nav/avatar-${catIndex}-${itemIndex}.${ext}`
    ...
    // Serialize to YAML and create blob
    const yamlContent = yaml.dump(navData, { lineWidth: -1, noRefs: true })
